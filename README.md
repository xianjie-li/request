## Introduce
使用axios发起请求，返回[err, res]形式的结果, 根据传入配置可进行缓存、集中的进行错误处理、loading、 挂token等。
...
* 对请求异常的自动化处理。
* 除了默认的状态码错误检测外，还可以通过response针对服务器约定的错误进行处理。
* 方便集成统一的请求loading与请求前操作（配置token等）。
...
interface CreateOptions {
  ...
  /** 接收服务端返回并根据约定的属性名返回一个boolean值帮助判定本次请求成功与否 */
...
/**
 * * 当服务端返回裸数据时，可以任意为serverMsgField设置一个字符值，此时根据状态码进行的错误反馈依然是可用的
 */
```
